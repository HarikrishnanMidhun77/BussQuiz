...
public class DBHandler extends SQLiteOpenHelper {
    ...
    private static final String DATABASE_NAME = "DB_Questions";//old DB_Random
    private static final String DATABASE_TABLE = "Table_QOD";//"Table_Random_Questions"
 ...
 private static final String PRIMARY_ENTRY="INSERT INTO "+ DATABASE_TABLE + "(" + KEY_QID+" , "+ KEY_Q_CAT+" , "+KEY_Q_SUB+ " , "+ KEY_DIFFICULTY +" , "+ KEY_Q_TYPE  + " , " + KEY_QUESTION + " , " + KEY_ANS1 + "  , " + KEY_ANS2 + "  , " + KEY_ANS3 + "  , " + KEY_ANS4 + "  , " + KEY_CORRECTANS + " , " + KEY_CORRECTANS2 + " , " + KEY_CORRECTANS3 + ","+ KEY_CORRECTANS4 + ","+ KEY_C_ANS_NO + ","+KEY_PIC_NAME+","+KEY_IS_SHOWN+" ) VALUES (1,'SST','SAS',1,0"+",'"+ qstn1 +"',"+"'35%','30%','90%','40%',null,null,null,'40%',1,'pic1',0)";
   ...
   // SQLiteDatabase odb = this.getReadableDatabase();
    ...
    {
        odb = this.getReadableDatabase();
        Cursor cursor =odb.rawQuery("select * from " + DATABASE_TABLE + " WHERE "+KEY_DIFFICULTY +" = "+ lev +" LIMIT 1", null);
        return cursor;
    }
  ...
  /*  public Cursor getRandom(){
    ...
    {
        odb = this.getReadableDatabase();
        String ChQry1="SELECT * FROM "+DATABASE_TABLE+" WHERE IS_SHOWN=1";
        String ChQry2="SELECT * FROM "+DATABASE_TABLE;
        Cursor cursor1 = odb.rawQuery(ChQry1, null);
        Cursor cursor2 = odb.rawQuery(ChQry2, null);
        int TrowCount = cursor2.getCount();
        int UsedRowCount=cursor1.getCount();
        cursor1.close();
        cursor2.close();
        odb.close();
        if(TrowCount-2==UsedRowCount)
             return true;
        else
            return  false;
    }
    ...
    {
        String qry="UPDATE "+ DATABASE_TABLE+" SET IS_SHOWN=1 WHERE "+ KEY_QID +" = "+ qid;
         odb.execSQL(qry);
        odb.close();
    }
